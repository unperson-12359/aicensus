...
export function ToolCardFeatured({ tool }: ToolCardFeaturedProps) {
  ...
  return (
    <article className="bento-tile group relative flex h-full flex-col overflow-hidden p-5 sm:p-6 hover:border-white/30">
      <Link
      ...
      <div className="pointer-events-none relative z-10 flex items-start justify-between">
        <ToolLogo
          src={logoSrc}
          name={tool.name}
          className="h-14 w-14 rounded-2xl border border-white/10 text-lg sm:h-16 sm:w-16 sm:text-xl lg:h-14 lg:w-14 lg:text-lg"
        />
      ...
      <div className="pointer-events-none relative z-10 mt-4 flex-1 sm:mt-6 lg:mt-4">
        <div className="flex items-center gap-2">
          <h3 className="truncate text-2xl font-semibold tracking-hero text-foreground sm:text-3xl lg:text-2xl">
            {tool.name}
          </h3>
          {tool.is_verified && <VerifiedBadge showLabel={false} size="sm" />}
        </div>
        <p className="mt-3 max-w-xl text-base leading-relaxed text-muted-foreground line-clamp-3 lg:mt-2 lg:text-sm lg:line-clamp-2">
          {tool.tagline}
        </p>
      </div>

      <div className="pointer-events-none relative z-10 mt-4 flex flex-wrap items-center gap-3 border-t border-white/10 pt-4 sm:mt-6 sm:pt-5 lg:mt-4 lg:pt-4">
        {tool.categories && (
